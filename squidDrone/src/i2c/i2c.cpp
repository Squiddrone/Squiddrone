...
namespace i2c
{
  ...
  auto I2C::read(uint8_t address, const std::vector<uint8_t>& data, int timeout) noexcept -> std::tuple<I2CStatus, std::vector<uint8_t>> {
    auto result = std::vector<uint8_t>{0, 0, 0};
    return {I2CStatus::I2C_TRANSACTION_FAILED, result};
